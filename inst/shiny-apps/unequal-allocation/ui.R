...
choices <- c("PBD", "BUD", "MWUD", "DL", "DBCD", "MinQD", "MaxEnt")
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      wellPanel(
      fluidRow(
        div(style="display:inline-block",checkboxInput("proc11_check", "CRD", 1))
      ),
      fluidRow(
        div(style="display:inline-block",checkboxInput("proc12_check", "MaxEnt (1)", 1))
      ),
      fluidRow(
    ...
    mainPanel(
      ...
      tabsetPanel(
        tabPanel("Proportions Boxplots",
                 p(),
                 p("The plots below show distributions of subjects' allocation proportions per treatment arm."),
                 p(),
                 plotOutput("allocation_boxplot", width = "100%", height = "1000px")),

        tabPanel("Unconditional Allocation Probability",
                 p(),
                 p("The plot below shows unconditional allocation probability per treatment for selected designs."),
                 p(),
                 plotOutput("probability_plot", width = "100%", height = "1000px")),

        tabPanel("Forcing Index/Momentum of Probability Mass",
                 p(),
                 p("The plots below show average forcing index (AFI) and cumulative momentum of probability mass (CMPM) for selected designs."),
                 p(),
                 plotOutput("op_plot", width = "100%", height = "1000px")),
        tabPanel("Overall Performance",
                 p(),
                 p("The table below shows the overall performance of design. The lower the G value, the better the design is."),
                 p(),
                 tableOutput("ovp_table"),
                 p("The plot below shows overall performance of designs vs allocation step."),
                 plotOutput("ovp_plot", width = "100%", height = "1000px")),
        tabPanel("Type I error",
                 p(),
                 p("The plots below show the type I error/power control estimated through the simulations."),
                 p(),
